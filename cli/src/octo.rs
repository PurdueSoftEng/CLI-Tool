...
pub async fn get_issue(
    ...
    owner: String,
    repo: String,
) -> Result<Page<octocrab::models::issues::Issue>, octocrab::Error> {
...
pub async fn get_issues(
    ...
    repo: String,
    t: i64,
) -> Result<Vec<Vec<octocrab::models::issues::Issue>>> {
    let route = format!("repos/{owner}/{repo}/issues/");
    let octo = Octocrab::builder().personal_token(token).build().unwrap();
...
pub async fn get_commits(
    ...
    repo: String,
    t: f64,
) -> Result<Vec<Vec<RepoCommit>>, octocrab::Error> {
    let route = format!("repos/{owner}/{repo}/commits");
    let octo = Octocrab::builder().personal_token(token).build().unwrap();
    let mut page = octo
        .repos(owner, repo)
        .list_commits()
        .since(Utc::now() - Duration::days(t as i64))
        .send()
        .await?;
    ...
    loop {
        ...
        match octo.get_page(&page.next).await {
...
pub async fn get_bugs(
    ...
    owner: String,
    repo: String,
) -> Result<Vec<octocrab::models::issues::Issue>, octocrab::Error> {
    let octo = Octocrab::builder().personal_token(token).build().unwrap();
...
pub async fn get_all_commits(
    ...
    loop {
        ...
        page = match octo.get_page::<RepoCommit>(&page.next).await {
            Ok(Some(next_page)) => next_page,
            Ok(None) => break,
            Err(_) => break,
        }
...
pub async fn get_duration_between_first_and_last_commit(
    token: String,
    owner: String,
    repo: String,
) -> Result<f64, octocrab::Error> {
    let octo = Octocrab::builder().personal_token(token).build().unwrap();
    ...
    let first_commit_date = NaiveDate::parse_from_str(
        &format!(
            "{}-{}-{}",
            &first_commit_sha[0..4],
            &first_commit_sha[4..6],
            &first_commit_sha[6..7]
        ),
        "%Y-%m-%d",
    )
    .unwrap();
    ...
    let most_recent_commit_date = NaiveDate::parse_from_str(
        &format!(
            "{}-{}-{}",
            &most_recent_commit_sha[0..4],
            &most_recent_commit_sha[4..6],
            &most_recent_commit_sha[6..7]
        ),
        "%Y-%m-%d",
    )
    .unwrap();
...
pub async fn get_issue_response_times(
    token: String,
    owner: String,
    repo: String,
) -> Result<Vec<f64>, octocrab::Error> {
    ...
    let max_time_to_response = 120 * 24 * 60 * 60; // 120 days in seconds
...
pub async fn get_contributors_with_percentages(
    token: String,
    owner: String,
    repo: String,
) -> Result<Vec<(octocrab::models::repos::Contributor, i32, f32)>, octocrab::Error> {
    let octo = Octocrab::builder().personal_token(token).build().unwrap();
    ...
    let total_contributions: i32 = contributor_list
        .iter()
...
pub async fn are_all_issues_closed(
    octo: Octocrab,
    owner: String,
    repo: String,
) -> Result<i32, octocrab::Error> {
...
pub async fn has_testing_suite(
    octo: Octocrab,
    owner: String,
    repo: String,
) -> Result<i32, octocrab::Error> {
...
pub async fn check_number_of_releases(
    token: String,
    owner: String,
    repo: String,
) -> Result<usize, octocrab::Error> {
    let octo = Octocrab::builder().personal_token(token).build().unwrap();
    //let temp = octo.repos(owner.clone(), repo.clone()).list_forks().send().await?;
    let octocrab = octocrab::Octocrab::default();
    let page = octocrab
        .repos("PurdueSoftEng", "CLI-Tool")
        .releases()
        .list()
        // Optional Parameters
        .per_page(100)
